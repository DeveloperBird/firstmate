...
An abnormal watcher exit or an unconfirmed-timeout termination is persisted to `state/.watch-crash.log` (size-capped, never deleted) before the ephemeral per-arm output temp file is removed, capturing the child's exit code and its last stdout/stderr; both this log and the watcher's own `state/.watch-triage.log` share one cap-and-append helper, `fm_capped_log_append` in `bin/fm-wake-lib.sh`.
...
Zellij's container shape is simpler than herdr's: one shared `firstmate` session, one tab per task, with no per-home workspace split; visible tab titles are scoped by the active home label plus a short hash of the resolved `FM_ROOT` path while callers keep using `fm-<id>`.
