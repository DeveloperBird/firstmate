...
If your session name is a bare small integer (tmux's own default naming scheme when a session is started without `-s`, e.g. `0`), append a trailing colon to the manual commands above (`tmux list-windows -t <session-name>:`) - otherwise tmux can parse the bare number as a window-index target in your current session instead of a session name. Firstmate's own internal tmux calls already do this (`bin/backends/tmux.sh`).
